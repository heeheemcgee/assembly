...
func CopyGraph(graph map[string][]string) map[string][]string {
    ...
    for key, value := range graph {
        ...
        n := copy(newArr, value)
        if (n != len(value)) {panic("Something wrong happend when copying graph.")}
        newGraph[key] = newArr
    }
    return newGraph
}
...
func GraphEq(graph1, graph2 map[string][]string) bool {
    for key1, value1 := range graph1 {
        ...
        if (!StrSliceEq(value1, value2)) {return false}
    }
...
//It returns if slice1 == slice2, assuming elements in same order.
